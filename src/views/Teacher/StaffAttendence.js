...
const TeacherAttendanceScreen = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      const response = await axios.get('http://server3.khaugalli.info/api/teacherattendance/get');
  ...
  useEffect(() => {
    ...
    const filteredData = attendanceData.filter((attendance) => {
      const teacherMatch = attendance.teacher.name.includes(teacherFilter) || teacherFilter === '';
    ...
    <table style={styles.table}>
      <thead>
        <tr style={styles.headerRow}>
          <th style={styles.headerCell}>Teacher</th>
          <th style={styles.headerCell}>Date</th>
          <th style={styles.headerCell}>Punch In</th>
          <th style={styles.headerCell}>Punch Out</th>
        </tr>
      </thead>
      <tbody>
        {filteredAttendance.map((item) => (
          <tr key={item._id}>
            <td style={styles.cell}>{item.teacher.name}</td>
            <td style={styles.cell}>{new Date(item.date).toLocaleDateString()}</td>
            <td style={styles.cell}>{item.punchIn}</td>
            <td style={styles.cell}>{item.punchOut}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={{marginTop:'20px'}}>Staff Attendance Sheet </p>
